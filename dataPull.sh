#!/bin/bash
#Crypto currencies config
api_key="44CD512C-79C1-4A5F-A6E0-5814C1D2E0F1"
currentPath=$(pwd)
output_dir="${currentPath}/pulledData/"
...
#Goud config
goud_url="https://www.goldapi.io/api/XAU/EUR" 
goud_api_key="goldapi-pehd1rlpyj9ww8-io"
...
get_gold_data(){
  ...
  local error_file="${output_dir}error-${tijd}.json"
  curl_pull() {
    curl -X GET "${goud_url}" -H "x-access-token: ${goud_api_key}" >"${data_file}" 2>"${error_file}"
  }

  if curl_pull; then
    rm -f "${error_file}"
    echo "De goudprijs is opgehaald en opgeslagen in ${data_file}."
  else
    rm -f "${data_file}"
    echo "Er is een fout opgetreden bij de Goud API-oproep."
    echo "Bekijk het foutbestand voor meer informatie: ${error_file}."
  fi
}
